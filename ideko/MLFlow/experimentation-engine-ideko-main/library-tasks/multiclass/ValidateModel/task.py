...
def post_result(file, model_result, myresultMap):
    ...
    if model_result == 0:
        ...
        myresultMap.put(file, "mechanical anomalous")

    elif model_result == 2:
        logger.info("post_result(): The model has classified the data as anomalous")
        myresultMap.put(file, "electrical anomalous")
...
for folder in labels.keys():
    ...
    for file in files:
        ...
        print("File %s, true label %d, predicted label %d" %(file, y, prediction))
